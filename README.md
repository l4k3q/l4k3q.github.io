...
在 front matter 里加一行 `pin: true`，文章会固定在首页列表最上方，带"置顶"标记。
多篇置顶时，置顶文章之间仍按日期倒序排列。想去掉置顶删掉这一行即可。

### 标签分类

在 front matter 的 `tags:` 下列出任意个标签。首页列表和文章页都会显示标签，
点标签会跳到 `/tags/` 归档页，那里自动按标签分组列出所有文章（无需手动维护）。
